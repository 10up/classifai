...
- **`classifai_openai_usage_refresh_interval_minutes`** (filter): Change how often usage is fetched from the OpenAI Costs API. Default is 15 minutes. Pass an integer (minutes).
...
- **`classifai_openai_usage_updated`** (action): Fires after usage has been refreshed from the API. Arguments: `$usage` (cached array), `$raw` (summary of API response). Use for logging or custom alerts.
