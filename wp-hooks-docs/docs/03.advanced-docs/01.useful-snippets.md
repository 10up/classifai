...
## OpenAI usage and pricing hooks

When [OpenAI pricing usage](01.get-started/03.pricing) is enabled, the following hooks are available:

- **`classifai_openai_usage_refresh_interval_minutes`** (filter): Change how often usage is fetched from the OpenAI Costs API. Default is 900 (15 minutes in seconds). Pass an integer in **seconds** (e.g. 600 for 10 minutes). The value is used as the Action Scheduler recurring interval.
- **`classifai_openai_cached_usage`** (filter): Modify or extend the cached usage array (`this_month_total`, `ytd_total`, `all_time_total`, `currency`, `last_updated`) before it is displayed in the dashboard widget or settings.
- **`classifai_openai_can_make_request`** (filter): Set to `false` to block all OpenAI API requests (e.g. when a hard spending limit is reached). Return `true` to allow requests. Used by the hard-limit gate; themes/plugins can also use this to disable OpenAI for other reasons.
- **`classifai_openai_admin_api_key`** (filter): Override the admin API key used for the OpenAI Costs API (e.g. from environment or a secrets manager). Arguments: `$admin_api_key` (string), `$option` (full pricing option array). Return the key to use.
- **`classifai_openai_usage_updated`** (action): Fires after usage has been refreshed from the API. Arguments: `$usage` (cached array with `this_month_total`, `ytd_total`, `all_time_total`, `currency`, `last_updated`), `$raw` (reserved for future use; currently an empty array). Use for logging or custom alerts.
- **`classifai_openai_soft_threshold_exceeded`** (action): Fires when the soft spending threshold is exceeded (before the built-in email is sent). Arguments: `$pricing` (full pricing option array), `$amount` (float, current usage for the scope), `$threshold` (float), `$scope` (string: `current_month`, `year_to_date`, or `all_time`). Use for custom notifications or logging.
- **`classifai_openai_hard_threshold_exceeded`** (action): Fires when the hard spending threshold is exceeded (after OpenAI features are disabled via the hard-limit option, before the built-in email is sent). Arguments: `$pricing` (full pricing option array), `$amount` (float, current usage for the scope), `$threshold` (float), `$scope` (string: `current_month`, `year_to_date`, or `all_time`). Use for custom notifications or logging.
