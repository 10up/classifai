---
id: feature-configuration
title: "Feature Configuration"
sidebar_label: "Feature Configuration"
---

# Feature Configuration

Welcome to the ClassifAI Feature Configuration guide. This section provides detailed configuration instructions for all available features in ClassifAI.

## Language Processing Features

- [Classification](./01.classification.md) - Configure content classification using IBM Watson or OpenAI Embeddings
- [Title Generation](./02.title-generation.md) - Set up automated title generation for your content
- [Excerpt Generation](./03.excerpt-generation.md) - Configure automatic excerpt generation
- [Content Generation](./04.content-generation.md) - Set up AI-powered content generation
- [Writing Tools](./05.writing-tools.md) - Configure content length adjustment features and fix grammar
- [Key Takeaways](./06.key-takeaways.md) - Set up automatic key points extraction
- [Text to Speech](./07.text-to-speech.md) - Configure text-to-speech conversion
- [Audio Transcripts Generation](./08.audio-transcripts-generation.md) - Set up audio transcription
- [Moderation](./09.moderation.md) - Configure content moderation features
- [Smart 404](./10.smart-404.md) - Set up intelligent 404 handling
- [Term Cleanup](./11.term-cleanup.md) - Configure taxonomy term management
- [Recommended Content](./18.recommended-content.md) - Set up content recommendation features

## Image Processing Features

- [Descriptive Text Generator](./12.descriptive-text-generator.md) - Configure automatic alt text generation
- [Image Tags Generator](./13.image-tags-generator.md) - Set up image tagging
- [Image Cropping](./14.image-cropping.md) - Configure smart image cropping
- [Image Text Extraction](./15.image-text-extraction.md) - Set up OCR features
- [Image Generation](./16.image-generation.md) - Configure AI image generation

## Document Processing Features

- [PDF Text Extraction](./17.pdf-text-extraction.md) - Configure PDF content extraction

Each feature can be configured independently based on your needs. Click on the respective links to access detailed configuration instructions for each feature.
