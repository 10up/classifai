...
1. Version bump: Bump the version number in `classifai.php`, `config.php`, `package-lock.json`, `package.json`, and `readme.txt` if it does not already reflect the version being released.  For major version releases, also update in `wp-hooks-docs/docs/01.get-started/01.installation.md`.
