...
class ImageCropping extends Feature {
	...
	public function generate_smart_crops( array $metadata, int $attachment_id ): array {
		...
		if ( 'automatic_async' === $mode && $this->enqueue_async_image_job( $attachment_id, 'crop' ) ) {
			return $metadata;
		}
	...
	public function migrate_settings() {
		...
		if ( isset( $old_settings['smart_cropping_roles'] ) ) {
			$new_settings['roles'] = $old_settings['smart_cropping_roles'];
		}

		if ( isset( $old_settings['smart_cropping_users'] ) ) {
			$new_settings['users'] = $old_settings['smart_cropping_users'];
		}

		if ( isset( $old_settings['smart_cropping_user_based_opt_out'] ) ) {
			$new_settings['user_based_opt_out'] = $old_settings['smart_cropping_user_based_opt_out'];
		}
