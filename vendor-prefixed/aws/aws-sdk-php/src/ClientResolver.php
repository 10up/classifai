...
use ClassifaiVendor\Aws\Api\ApiProvider;
use ClassifaiVendor\Aws\Api\Service;
use ClassifaiVendor\Aws\Api\Validator;
use ClassifaiVendor\Aws\Auth\AuthResolver;
use ClassifaiVendor\Aws\Auth\AuthSchemeResolver;
use ClassifaiVendor\Aws\Auth\AuthSchemeResolverInterface;
use ClassifaiVendor\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use ClassifaiVendor\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use ClassifaiVendor\Aws\ClientSideMonitoring\Configuration;
use ClassifaiVendor\Aws\Configuration\ConfigurationResolver;
use ClassifaiVendor\Aws\Credentials\CredentialProvider;
use ClassifaiVendor\Aws\Credentials\Credentials;
use ClassifaiVendor\Aws\Credentials\CredentialsInterface;
use ClassifaiVendor\Aws\DefaultsMode\ConfigurationInterface as ConfigModeInterface;
use ClassifaiVendor\Aws\DefaultsMode\ConfigurationProvider as ConfigModeProvider;
use ClassifaiVendor\Aws\Endpoint\EndpointProvider;
use ClassifaiVendor\Aws\Endpoint\PartitionEndpointProvider;
use ClassifaiVendor\Aws\Endpoint\UseDualstackEndpoint\Configuration as UseDualStackEndpointConfiguration;
use ClassifaiVendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationInterface as UseDualStackEndpointConfigurationInterface;
use ClassifaiVendor\Aws\Endpoint\UseDualstackEndpoint\ConfigurationProvider as UseDualStackConfigProvider;
use ClassifaiVendor\Aws\Endpoint\UseFipsEndpoint\Configuration as UseFipsEndpointConfiguration;
use ClassifaiVendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationInterface as UseFipsEndpointConfigurationInterface;
use ClassifaiVendor\Aws\Endpoint\UseFipsEndpoint\ConfigurationProvider as UseFipsConfigProvider;
use ClassifaiVendor\Aws\EndpointDiscovery\ConfigurationInterface;
use ClassifaiVendor\Aws\EndpointDiscovery\ConfigurationProvider;
use ClassifaiVendor\Aws\EndpointV2\EndpointDefinitionProvider;
use ClassifaiVendor\Aws\Exception\AwsException;
use ClassifaiVendor\Aws\Exception\InvalidRegionException;
use ClassifaiVendor\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use ClassifaiVendor\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use ClassifaiVendor\Aws\Signature\SignatureProvider;
use ClassifaiVendor\Aws\Token\Token;
use ClassifaiVendor\Aws\Token\TokenInterface;
use ClassifaiVendor\Aws\Token\TokenProvider;
...
class ClientResolver
{
    ...
    {
        if ($value === false) {
            $value = new Configuration(false, \ClassifaiVendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \ClassifaiVendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \ClassifaiVendor\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _apply_endpoint_provider($value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            if ($value instanceof \ClassifaiVendor\Aws\EndpointV2\EndpointProviderV2) {
            ...
            $args['region'] = \ClassifaiVendor\Aws\strip_fips_pseudo_regions($args['region']);
    ...
    {
        if ($value !== false) {
            $list->interpose(new TraceMiddleware($value === true ? [] : $value, $args['api']));
        }
    }
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \ClassifaiVendor\Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
    ...
    public static function _apply_suppress_php_deprecation_warning($value, &$args)
    {
        if ($value) {
            ...
            $args['suppress_php_deprecation_warning'] = \ClassifaiVendor\Aws\boolean_value(getenv("AWS_SUPPRESS_PHP_DEPRECATION_WARNING"));
        } elseif (!empty($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \ClassifaiVendor\Aws\boolean_value($_SERVER["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        } elseif (!empty($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"])) {
            $args['suppress_php_deprecation_warning'] = \ClassifaiVendor\Aws\boolean_value($_ENV["AWS_SUPPRESS_PHP_DEPRECATION_WARNING"]);
        }
    ...
    public static function _default_endpoint(array &$args)
    {
        ...
        $serviceIdentifier = \ClassifaiVendor\Aws\manifest($args['service'])['serviceIdentifier'];
    ...
    private static function isValidService($service)
    {
        ...
        $services = \ClassifaiVendor\Aws\manifest();
