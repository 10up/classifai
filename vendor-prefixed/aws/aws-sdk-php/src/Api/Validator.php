...
class Validator
{
    ...
    {
        if (!is_array($value)) {
            $this->addError('must be an array. Found ' . \ClassifaiVendor\Aws\describe_type($value));
            return;
        }
    ...
    private function check_blob(Shape $shape, $value)
    {
        ...
        if (!isset($valid[$type])) {
            if ($type != 'object' || !method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'ClassifaiVendor\GuzzleHttp\Stream\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \ClassifaiVendor\Aws\describe_type($value));
            }
        }
    }
    ...
    {
        if (!is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \ClassifaiVendor\Aws\describe_type($value));
            return;
        }
    ...
    {
        if (!is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \ClassifaiVendor\Aws\describe_type($value));
        }
    }
    private function check_string(Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \ClassifaiVendor\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \ClassifaiVendor\Aws\describe_type($value));
            return;
        }
    ...
    private function checkAssociativeArray($value)
    {
        ...
        if (!$isAssociative) {
            $this->addError('must be an associative array. Found ' . \ClassifaiVendor\Aws\describe_type($value));
            return false;
        }
        return true;
    }
