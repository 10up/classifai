...
use ClassifaiVendor\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
/**
...
class Transfer implements PromisorInterface
{
     ...
     *   callback accepts a single argument: ClassifaiVendor\Aws\CommandInterface $command.
    ...
    public function __construct(S3ClientInterface $client, $source, $dest, array $options = [])
    {
        ...
        $this->addContentMD5 = isset($options['add_content_md5']) && $options['add_content_md5'] === true;
    ...
    private function createDownloadPromise()
    {
        ...
        return (new \ClassifaiVendor\Aws\CommandPool($this->client, $commands, ['concurrency' => $this->concurrency, 'before' => $this->before, 'rejected' => function ($reason, $idx, Promise\PromiseInterface $p) {
            $p->reject($reason);
        }]))->promise();
    }
    private function createUploadPromise()
    {
        ...
        $files = \ClassifaiVendor\Aws\map($this->getUploadsIterator(), function ($file) {
    ...
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return \ClassifaiVendor\Aws\filter(\ClassifaiVendor\Aws\recursive_dir_iterator($this->sourceMetadata['path']), function ($file) {
                return !is_dir($file);
            });
        }
        return $this->source;
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = \ClassifaiVendor\Aws\map($files, function ($key) use ($listArgs) {
                return "s3://{$listArgs['Bucket']}/{$key}";
            });
            return \ClassifaiVendor\Aws\filter($files, function ($key) {
