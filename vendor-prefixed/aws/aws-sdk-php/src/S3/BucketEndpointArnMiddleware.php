...
use ClassifaiVendor\Aws\Arn\ObjectLambdaAccessPointArn;
...
use ClassifaiVendor\Aws\Arn\S3\OutpostsAccessPointArn;
use ClassifaiVendor\Aws\Arn\S3\MultiRegionAccessPointArn;
use ClassifaiVendor\Aws\Arn\S3\OutpostsArnInterface;
...
use ClassifaiVendor\Aws\Exception\UnresolvedEndpointException;
use ClassifaiVendor\Aws\S3\Exception\S3Exception;
...
class BucketEndpointArnMiddleware
{
    ...
    private function generateAccessPointHost(
        ...
        $region = \ClassifaiVendor\Aws\strip_fips_pseudo_regions($region);
     ...
     *
     * @param $arn
     * @return \ClassifaiVendor\Aws\Endpoint\Partition
     */
    private function validateArn($arn)
    {
        if ($arn instanceof AccessPointArnInterface) {
            ...
            if (!($clientPart->isRegionMatch($this->region, 's3'))) {
                $clientPart = $this->partitionProvider->getPartition(
                    \ClassifaiVendor\Aws\strip_fips_pseudo_regions($this->region),
                    's3'
                );
            }
            if (!$arn instanceof MultiRegionAccessPointArn) {
