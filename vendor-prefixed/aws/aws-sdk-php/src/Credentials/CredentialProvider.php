...
namespace ClassifaiVendor\Aws\Credentials;

use ClassifaiVendor\Aws;
use ClassifaiVendor\Aws\Api\DateTimeResult;
use ClassifaiVendor\Aws\CacheInterface;
use ClassifaiVendor\Aws\Exception\CredentialsException;
use ClassifaiVendor\Aws\Sts\StsClient;
...
class CredentialProvider
{
     ...
     * @see \ClassifaiVendor\Aws\Credentials\InstanceProfileProvider for $config details.
     */
     ...
     * @see \ClassifaiVendor\Aws\Credentials\EcsCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see \ClassifaiVendor\Aws\Credentials\AssumeRoleCredentialProvider for $config details.
     */
     ...
     * @param array $config Array of configuration data
     * @return callable
     * @see \ClassifaiVendor\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider for
     * $config details.
     */
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use ($profile, $filename) {
            ...
            $data = \ClassifaiVendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
    ...
    {
        $profileData = \ClassifaiVendor\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \ClassifaiVendor\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (file_exists($credFile)) {
            $profiles = \ClassifaiVendor\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }
        if (file_exists($configFile)) {
            $configProfileData = \ClassifaiVendor\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    private static function getSsoCredentials($profiles, $ssoProfileName, $filename, $config)
    {
        if (empty($config['ssoOidcClient'])) {
            ...
            $ssoOidcClient = new \ClassifaiVendor\Aws\SSOOIDC\SSOOIDCClient(['region' => $ssoSession['sso_region'], 'version' => '2019-06-10', 'credentials' => false]);
        } else {
            $ssoOidcClient = $config['ssoClient'];
        }
        $tokenPromise = new \ClassifaiVendor\Aws\Token\SsoTokenProvider($ssoProfileName, $filename, $ssoOidcClient);
    ...
    private static function getCredentialsFromSsoService($ssoProfile, $clientRegion, $accessToken, $config)
    {
        if (empty($config['ssoClient'])) {
            $ssoClient = new \ClassifaiVendor\Aws\SSO\SSOClient(['region' => $clientRegion, 'version' => '2019-06-10', 'credentials' => false]);
        } else {
