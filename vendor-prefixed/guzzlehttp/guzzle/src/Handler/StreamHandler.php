...
use ClassifaiVendor\GuzzleHttp\Promise\FulfilledPromise;
...
use ClassifaiVendor\GuzzleHttp\Utils;
...
class StreamHandler
{
    ...
    private static function triggerUnsupportedRequestOptionDeprecations(RequestInterface $request, array $options): void
    {
        if (
            ...
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "curl" request option to the stream handler is deprecated; guzzlehttp/guzzle 8.0 will reject this option because the stream handler ignores cURL options.');
        }

        if (\array_key_exists('expect', $options) && $options['expect'] !== false && $request->hasHeader('Expect')) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing the "expect" request option to the stream handler is deprecated when it adds an Expect header; guzzlehttp/guzzle 8.0 will reject this option because the stream handler does not support Expect: 100-Continue.');
        }
    }
    ...
    private static function triggerConflictingStreamContextOptionDeprecations(array $streamContext): void
    {
        ...
        foreach ($streamContext as $wrapper => $contextOptions) {
            ...
            foreach ($contextOptions as $option => $_) {
                ...
                classifaivendor_trigger_deprecation(
                    'guzzlehttp/guzzle',
                    '7.12',
                    \sprintf(
