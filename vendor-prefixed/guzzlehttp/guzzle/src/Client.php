...
use ClassifaiVendor\GuzzleHttp\Cookie\CookieJar;
use ClassifaiVendor\GuzzleHttp\Exception\GuzzleException;
use ClassifaiVendor\GuzzleHttp\Exception\InvalidArgumentException;
use ClassifaiVendor\GuzzleHttp\Handler\CurlShareHandleState;
use ClassifaiVendor\GuzzleHttp\Promise as P;
 ...
 * @final
 */
class Client implements ClientInterface, \ClassifaiVendor\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        if ($method !== $normalizedMethod) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::requestAsync() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
        ...
        if ($droppedHeaderNames !== [] && isset($options['_conditional'])) {
            $options['_conditional'] = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::caselessRemove($droppedHeaderNames, $options['_conditional']);
        }
        $body = $options['body'] ?? null;
        $version = self::normalizeProtocolVersion($options['version'] ?? '1.1');
        // Merge the URI into the base URI.
        $uri = $this->buildUri(\ClassifaiVendor\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    public function request(string $method, $uri = '', array $options = []): ResponseInterface
    {
        $normalizedMethod = \strtoupper($method);
        if ($method !== $normalizedMethod) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing a non-uppercase HTTP method to Client::request() is deprecated; guzzlehttp/guzzle 8.0 will preserve HTTP method casing. Pass an uppercase method explicitly if uppercase is required.');
            $method = $normalizedMethod;
        }
        $options[RequestOptions::SYNCHRONOUS] = true;
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \ClassifaiVendor\GuzzleHttp\Psr7\UriResolver::resolve(\ClassifaiVendor\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
    ...
    private static function warnAboutRequestLevelHandler(array $options): void
    {
        ...
        classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing the "handler" request option is deprecated; guzzlehttp/guzzle 8.0 will ignore request-level handlers. Configure the handler when creating the Client, or use a separate Client instance for requests that need a different handler.');
    }
    ...
    {
        classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s to request option "%s" is deprecated; guzzlehttp/guzzle 8.0 requires %s.', \get_debug_type($value), $option, $expected);
    }
    /**
    ...
    private function transfer(RequestInterface $request, array $options): PromiseInterface
    {
        ...
        if ('' === $protocolVersion) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with an empty protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            $request = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, ['version' => '1.1']);
        } elseif (!self::isProtocolVersionValid($protocolVersion)) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Sending a request with a malformed protocol version is deprecated; guzzlehttp/guzzle 8.0 will reject malformed protocol versions.');
        }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::streamFor($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
                ...
                case 'digest':
                    ...
                    break;
                case 'ntlm':
                    classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', 'Passing "ntlm" as the built-in auth type is deprecated; guzzlehttp/guzzle 8.0 will no longer apply NTLM through the "auth" request option. Configure NTLM with cURL HTTP authentication options instead.');
        ...
        $request = \ClassifaiVendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \ClassifaiVendor\GuzzleHttp\Psr7\MultipartStream) {
    ...
    {
        if ('' === $version) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing an empty "version" request option is deprecated; guzzlehttp/guzzle 8.0 will reject empty protocol versions.');
            return '1.1';
        }
