...
use ClassifaiVendor\GuzzleHttp\Handler\CurlHandler;
use ClassifaiVendor\GuzzleHttp\Handler\CurlMultiHandler;
use ClassifaiVendor\GuzzleHttp\Handler\CurlShareHandleState;
use ClassifaiVendor\GuzzleHttp\Handler\CurlVersion;
use ClassifaiVendor\GuzzleHttp\Handler\Proxy;
use ClassifaiVendor\GuzzleHttp\Handler\StreamHandler;
...
final class Utils
{
    ...
    {
        classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.12', '%s() is deprecated and will be removed in 8.0. Use get_debug_type() instead.', __METHOD__);
    ...
    public static function debugResource($value = null)
    {
        ...
        return \ClassifaiVendor\GuzzleHttp\Psr7\Utils::tryFopen('php://output', 'w');
    }
    /**
    ...
    public static function normalizeIdnConversionOption($value): ?int
    {
        ...
        if (\is_string($value) && \is_numeric($value) || \is_float($value) && \is_finite($value)) {
            classifaivendor_trigger_deprecation('guzzlehttp/guzzle', '7.11', 'Passing %s as the "idn_conversion" request option is deprecated; guzzlehttp/guzzle 8.0 will reject values that are not true, false, null, or an integer IDNA_* bitmask.', \get_debug_type($value));
            return (int) $value;
        }
