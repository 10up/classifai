describe( 'Image Generation (Together AI) Tests', () => {
	before( () => {
		cy.login();
		cy.visitFeatureSettings( 'image_processing/feature_image_generation' );
		cy.enableFeature();
		cy.selectProvider( 'togetherai_image' );
	...
	it( 'Can save settings', () => {
		cy.visitFeatureSettings( 'image_processing/feature_image_generation' );
		cy.selectProvider( 'togetherai_image' );
		cy.get( '#togetherai_api_key' ).clear().type( 'password' );
		cy.get( 'select#togetherai_image_number_of_images' ).select( '2' );
		cy.get( 'select#togetherai_image_image_size' ).select( '1024x1536' );
