...
test.describe( 'Image Generation (Google AI) Tests', () => {
	...
	test( 'Can save settings', async ( { classifaiUtils, page } ) => {
		...
		await classifaiUtils.selectProvider( 'googleai_images' );
		await page.locator( '#googleai_gemini_api_key' ).fill( 'password' );
		await page
			.locator( 'select#googleai_images_number_of_images' )
			.selectOption( '2' );
		await page
			.locator( 'select#googleai_images_aspect_ratio' )
			.selectOption( '16:9' );
	...
	test( 'Can generate images in the media modal', async ( {
		...
		await classifaiUtils.createPost( {
			title: 'Test Google AI Imagen post',
