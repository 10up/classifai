...
test.describe( 'Image Generation (Together AI) Tests', () => {
	...
	test( 'Can save settings', async ( { classifaiUtils, page } ) => {
		...
		await classifaiUtils.selectProvider( 'togetherai_image' );
		await page.locator( '#togetherai_api_key' ).fill( 'password' );
		await page
			.locator( 'select#togetherai_image_number_of_images' )
			.selectOption( '2' );
		await page
			.locator( 'select#togetherai_image_image_size' )
