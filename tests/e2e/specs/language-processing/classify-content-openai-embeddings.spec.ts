...
test.describe( '[Language processing] Classify Content (OpenAI) Tests', () => {
	test.beforeAll( async ( { browser, requestUtils } ) => {
		...
		await page
			...
			.selectOption( 'openai_embeddings' );
		// Save settings.
	...
	test( 'Can save OpenAI Embeddings "Language Processing" settings', async ( {
		classifaiUtils,
		page,
	} ) => {
		...
		// "Test" requires 80% confidence. At 81%, it does not apply.
	...
	test.skip( 'Can see the preview on the settings page', async ( {
		classifaiUtils,
		page,
	} ) => {
		await classifaiUtils.visitFeatureSettings(
		...
		await page.locator( '#get-classifier-preview-data-btn' ).click();

		// Check the term is received and visible.
		await expect( page.locator( '.tax-row--category' ) ).toBeAttached();
	} );

	test( 'Can create category and post and category will not get auto-assigned if feature turned off', async ( {
		classifaiUtils,
		editor,
		page,
		requestUtils,
		...
		await expect(
			panel.locator(
				'.editor-post-taxonomies__hierarchical-terms-list .editor-post-taxonomies__hierarchical-terms-choice:first-child label'
			)
	...
	test( 'Can see the enable button in a post (Classic Editor)', async ( {
		...
		await page.locator( '#title' ).fill( 'Embeddings test classic' );
		...
		await contentFrame
			...
			.fill( "This feature uses OpenAI's Embeddings capabilities." );
	...
	test( 'Can enable/disable content classification feature ', async ( {
		classifaiUtils,
	} ) => {
		await classifaiUtils.disableClassicEditor();
	...
	test( 'Can enable/disable content classification feature by role', async ( {
		classifaiUtils,
	} ) => {
	...
	test( 'Can enable/disable content classification feature by user', async ( {
		classifaiUtils,
	} ) => {
		// Disable admin role.
		await classifaiUtils.disableFeatureForRoles( 'feature_classification', [
		...
		await classifaiUtils.enableFeatureForUsers( 'feature_classification', [
			'admin',
		] );
	...
	test( 'User can opt-out content classification feature', async ( {
		classifaiUtils,
	} ) => {
		// Enable user based opt-out.
		await classifaiUtils.enableFeatureOptOut( 'feature_classification' );
