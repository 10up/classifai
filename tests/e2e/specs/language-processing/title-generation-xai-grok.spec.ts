...
test.describe( '[Language processing] Title Generation Tests', () => {
	...
	test( 'Can save OpenAI ChatGPT "Language Processing" title settings', async ( {
		...
		await page.locator( '#xai_grok_number_of_suggestions' ).fill( '1' );

		const setDefaultBtn = page.locator(
			'#classifai-prompt-setting-0 .actions-rows button.action__set_default'
		);
		const setDefaultText = ( await setDefaultBtn.textContent() ) || '';
		if ( setDefaultText.trim() === 'Set as default prompt' ) {
