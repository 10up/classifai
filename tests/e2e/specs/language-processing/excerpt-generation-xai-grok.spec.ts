...
test.describe( '[Language processing] Excerpt Generation Tests', () => {
	test.beforeAll( async ( { browser, requestUtils } ) => {
		...
		// Reset prompt 0 to default if it's not currently the default.
		const defaultBtn = page.locator(
			'#classifai-prompt-setting-0 .actions-rows button.action__set_default'
		);
		if ( await defaultBtn.count() ) {
			const btnText = ( await defaultBtn.textContent() ) || '';
			if ( btnText.trim() === 'Set as default prompt' ) {
	...
	test( 'Can save Azure OpenAI "Language Processing" settings', async ( {
		...
		await expect( page.locator( '#classifai-logo' ) ).toBeVisible();
		await classifaiUtils.selectProvider( 'xai_grok' );
		await page.locator( '#xai_grok_api_key' ).fill( 'password' );
