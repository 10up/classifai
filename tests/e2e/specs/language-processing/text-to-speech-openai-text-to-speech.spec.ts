...
test.describe( '[Language Processing] Text to Speech (OpenAI) Tests', () => {
	test.beforeAll( async ( { browser, requestUtils } ) => {
		...
		await page
			...
			.selectOption( 'openai_text_to_speech' );

		await page
			.locator( '#openai_text_to_speech_tts_model' )
			.selectOption( 'gpt-4o-mini-tts' );
		...
		await page
			.locator( '#openai_text_to_speech_voice' )
			.selectOption( 'ash' );
	...
	test( 'Can see the enable button in a post (Classic Editor)', async ( {
		...
		await page
			.locator( '#content' )
			.fill( "This feature uses OpenAI's Text to Speech capabilities." );
