...
test.describe( '[Language processing] Resize Content Tests', () => {
	test.beforeAll( async ( { browser, requestUtils } ) => {
		...
		await page
			...
			.selectOption( 'googleai_gemini_api' );
	...
	test( 'Resize content feature can grow and shrink content', async ( {
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.classifai-content-resize__result-table tbody tr:first-child .classifai-content-resize__grow-stat', { hasText: '+8 words' } )
		).toBeVisible();
		await expect(
			page.locator( '.classifai-content-resize__result-table tbody tr:first-child .classifai-content-resize__grow-stat', { hasText: '+49 characters' } )
		).toBeVisible();
		await page
			.locator(
		...
		await page
			...
			.click();
		await expect(
			page.locator( '.classifai-content-resize__result-table tbody tr:first-child .classifai-content-resize__shrink-stat', { hasText: '-5 words' } )
		).toBeVisible();
		await expect(
			page.locator( '.classifai-content-resize__result-table tbody tr:first-child .classifai-content-resize__shrink-stat', { hasText: '-27 characters' } )
		).toBeVisible();
		await page
			.locator(
				'.classifai-content-resize__result-table tbody tr:first-child button'
